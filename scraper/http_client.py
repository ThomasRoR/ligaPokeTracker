...
class ScraperHTTPClient:
    ...
    def _init_session(self):
        ...
        try:
            ...
            # Conecta na porta de depuração do Chrome que foi aberta pelo .bat
            # Como é o navegador REAL do usuário (com todo o histórico e cookies), o Cloudflare deixa passar direto!
            self._browser = self._playwright.chromium.connect_over_cdp("http://localhost:9222")
            
            # Pega o contexto padrao (a primeira aba/janela)
            self._context = self._browser.contexts[0]
            self._page = self._context.new_page()
            
            # Stealth ainda é util para esconder o automator
            Stealth().apply_stealth_sync(self._page)
            
            logger.info("Playwright connected to REAL Chrome browser successfully.")
    ...
    def fetch_expansion_page(self, expansion_code: str, page: int = 1) -> str:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            try:
                ...
                is_cloudflare = "Just a moment..." in content or "cloudflare" in content.lower() or "Desafio" in content or "Verificando" in content
                
                if response and response.status == 403 and not is_cloudflare:
